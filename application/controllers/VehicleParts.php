...
// AJAX: Get vehicles for dropdown
    public function get_vehicles_ajax() {
        $vehicles = $this->VehicleParts_model->get_vehicles();
        $data = [];
        foreach($vehicles as $v){
            // Return [id, other_data..., Vehicle Number] => adjust according to your model
            $data[] = [$v->idvehicledetails, $v->Vehicle_Make, $v->Vehicle_Model, $v->Chassis_No, $v->Vehicle_Num];
        }
        echo json_encode(['data' => $data]);
    }

    // AJAX: Get hire nos by vehicle/date
    public function getHireNosByVehicleDate() {
        $vehicleNum = $this->input->post('vehicleNum');
        $startDate  = $this->input->post('startDate');
        $endDate    = $this->input->post('endDate');

        $hires = $this->VehicleParts_model->get_hireNos($vehicleNum, $startDate, $endDate);
        echo json_encode(['data' => $hires]);
    }
...
public function averageFuelReport()
{
    ...
    $data = $this->VehicleParts_model->getAverageFuelData(
        $vehicleNum,
        $startDate,
        $endDate,
        $hireNo
...
public function getHireNosByVehicleDate1() {
    $vehicleNum = $this->input->post('vehicleNum');
    $startDate  = $this->input->post('startDate');
    $endDate    = $this->input->post('endDate');
