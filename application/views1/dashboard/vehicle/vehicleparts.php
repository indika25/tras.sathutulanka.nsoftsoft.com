...
<head>
    ...
    #partsTable td {
        ...
        font-size: 14px;
        /* Optional: also reduces height visually */
    }
...
<body class="bg-light">
    <div class="container-fluid">
        ...
        <!-- Modal -->
        <div class="modal fade" id="vehiclePartModal" tabindex="-1" role="dialog"
            aria-labelledby="vehiclePartModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl" role="document">
                <!-- xl for larger form -->
                <div class="modal-content">
                    <div class="modal-header bg-dark text-white">
                        <h5 class="modal-title" id="vehiclePartModalLabel"><i class="fas fa-tools mr-2"></i> Service
                            Details</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="addnewbtn" type="button" class="btn"
                            style="background-color: orange; color: white;">Clear</button>

                        <button id="saveBt" type="submit" form="vehiclePartForm" class="btn btn-success">Save Part
                            Info</button>
                    </div>
                </div>
            </div>
        ...
        $j(document).ready(function() {
            ...
            var table = $j('#partsTable').DataTable({
                ...
                columnDefs: [{
                    ...
                    }, // Hide ID column
                    {
                        targets: 1,
                        visible: false
                    }, // Row number column hidden (you may show if needed)
                    {
                        targets: 2
                    },
                    {
                ...
                success: function(response) {
                    ...
                    if (response.data && Array.isArray(response.data)) {
                        ...
                        $j.each(response.data, function(index, vehicle) {
                            ...
                            if (!addedIds.has(id)) {
                                select.append(
                                    `<option value="${id}">${vehicleNum}</option>`);
                                addedIds.add(id);
                            }
                        });
                    }
            ...
            $j('#vehiclePartForm').on('submit', function(e) {
                ...
                $('#savePartBtn').prop('disabled', true);
                ...
                $j.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
                                ...
                                let installDate = $j('input[name="last_checked"]')
                                    .val();
                    ...
                    complete: function() {
                        // ✅ ALWAYS release lock
                        isSubmitting = false;
                        $('#savePartBtn').prop('disabled', false);
                    }
                });
            });
