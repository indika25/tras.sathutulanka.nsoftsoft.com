...
#rentVehicleModal .modal-content {
 ...
 #materialUnit {
    width: 100% !important; /* full width of td */
    max-width: 150px;       /* max width you want */
  }
...
.dataTables_filter input {
  ...
  .ui-autocomplete {
  z-index: 3000 !important; /* ensure it shows above modals */
  max-height: 200px;
  overflow-y: auto;
  overflow-x: hidden;
...
<div class="container-fluid mt-3 px-2">
    ...
    <button id="addExpenses" type="button" class="btn text-white mr-2" style="background-color: #e74c3c;">
  <i class="fas fa-money-bill-wave"></i> Add Expenses
        ...
        <i class="fas fa-oil-can"></i> Add Diesel Levels
        </button>
...
<!-- 🚗 Vehicle Rent Modal -->
<div class="modal fade" id="rentVehicleModal" tabindex="-1" role="dialog" aria-labelledby="rentVehicleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document"> <!-- modal-xl for wider form -->
        <div class="modal-content">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title" id="rentVehicleModalLabel">
                    <i class="fas fa-tools mr-2"></i> Vehicle Hire Service
                </h5>
                <button type="button" class="close text-white btn-danger" data-dismiss="modal" aria-label="Close">
  ...
  <!-- <div class="input-group">
    <select id="hire_start_location" name="hire_start_location" class="form-control" required>
      <option value="">Select Hire Start Location</option>
           ...
           <input type="text" name="hire_start_location" class="form-control" style="text-transform: uppercase;" required>
  ...
  <!-- <div class="input-group">
    <select id="hire_end_location" name="hire_end_location" class="form-control" required>
      <option value="">Select Hire End Location</option>
          ...
          <input type="text" name="hire_end_location" class="form-control" style="text-transform: uppercase;">
...
<div class="col-md-3 mb-3">
        ...
        <input type="number" id="differencemileage" name="differencemileage" class="form-control" required readonly>
    </div>


   <div class="col-md-3 mb-3">
     <label for="oillevel">Engine Diesel Level</label>
     <div class="input-group">
...
<div class="col-md-3 mb-3">
  <div class="input-group">
    <table class="table table-bordered table-sm align-middle" id="materialTable">
     ...
     <tr>
  <td>
    <input type="text" id="materialName" class="form-control" placeholder="Name" style="width: 80px;" />
  </td>
  <td>
    <select name="materialUnit" id="materialUnit" class="form-control form-select-sm" style="width: 100px;">
  ...
  <td>
    <input type="number" id="materialAmount" class="form-control" placeholder="Amount" min="1" style="width: 80px;" />
  </td>
  <td class="text-center">
    <button type="button" id="addMaterialBtn" class="btn btn-success btn-sm" title="Add Material">
...
<div class="col-md-6 mb-3" style="margin-left:100px">
  ...
  <option value="">-- Select Expense --</option>
          ...
          <td>
            <input type="number" id="expenseAmount" class="form-control" placeholder="Amount" min="0" />
          </td>
          <td class="text-center">
            <button type="button" id="addExpenseBtn" class="btn btn-success btn-sm" title="Add Expense">
              <i class="fas fa-plus"></i>
            </button>
          </td>
        </tr>
      ...
      <div class="modal-body">
        <form id="expenseForm">
          <div class="form-group">
           <input type="text" hidden class="form-control" id="expenseIdModel" name="expenseId" placeholder="Enter expense ID">
          </div>
          <div class="form-group">
            <label for="expenseNameModel">Expense Name</label>
            <input type="text" class="form-control" id="expenseNameModel" name="expenseNameModel" placeholder="Enter expense name">
          </div>
        </form>
      </div>
...
<!-- Diesel Levels Modal -->
<div class="modal fade" id="oilLevelModal" tabindex="-1" role="dialog" aria-labelledby="oilLevelModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      <form id="oilLevelForm">
        ...
        <div class="modal-body">
          <!-- Input fields -->
          <div class="form-group">
            <label for="levelValue">Level Value (e.g., 0.5, 1.0)</label>
            <input type="number" step="0.1" class="form-control" id="levelValue" name="level_value" required>
          </div>
          <div class="form-group">
            <label for="levelLabel">Label (e.g., 1.0 L, 5 L Full)</label>
            <input type="text" class="form-control" id="levelLabel" name="label" required>
          </div>
...
var expensetable = $j('#expenseTablemodel').DataTable({
     ...
     dataSrc: function (json) {
      console.log('Raw AJAX response:', json);     // Full response
      console.log('Data source (json.data):', json.data); // The data array used by DataTables
      return json.data;
    },
...
$.ajax({
    ...
    success: function(response) {
        if (response.status === 'success') {
            ...
            select.empty(); // clear existing options
            select.append('<option value="">Select Start Diesel Level (L)</option>');
            $.each(response.data, function(index, oil) {
                select.append('<option value="'+oil.level_value+'">'+oil.label+'</option>');
            });
        } else {
            alert('Failed to load Diesel Levels.');
        }
    ...
    success: function (response) {
        ...
        if (response.customers && Array.isArray(response.customers)) {
            ...
            $j.each(response.customers, function (index, customer) {
                let id = customer.id;
                let name = customer.customer_name; // Or use customer.display_name
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                $.each(response.data, function(index, location) {
                    select.append('<option value="' + location.id + '">' + location.location_name + '</option>');
                });

               select2.find('option:not(:first)').remove(); // Keep default
                $.each(response.data, function(index, location) {
                    select2.append('<option value="' + location.id + '">' + location.location_name + '</option>');
                });
...
$j(document).on('input', 'input[name="mileage"], input[name="endmileage"]', function () {
    ...
    if (!isNaN(start) && !isNaN(end)) {
      ...
      $j('#differencemileage').val(difference); // if you have a <span id="mileageDifference">
    }
  });
...
$('#oilLevelsTable').on('click', '.deleteOilBtn', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('<?= base_url("RentVehicle/delete_oil_level") ?>', { id: id }, function(response) {
                if (response.status === 'success') {
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    ...
                    if (!isNaN(rentRate)) {
                        ...
                        $('input[name="rent_amount"]').val(totalAmount.toFixed(2)); // round to 2 decimals
                    } else {
...
if ($j('#hireno').val().trim() === '') {
    ...
    $j('#materialBody tr').each(function () {
        ...
        if (unit && !isNaN(qty)) {
            materials.push({ name, unit, qty,amount });
        }
    });
    ...
    if (id && exname && !isNaN(examount)) {
        Expenses.push({ id, exname, examount });
    }
    });

    // 👉 Add materials and expenses to form data
    formData.push({ name: 'materials', value: JSON.stringify(materials) });
    formData.push({ name: 'expenses', value: JSON.stringify(Expenses) });
...
if ($j(`#vehicle_id option[value="${rowData.vehicle_number}"]`).length === 0) {
    $j('#vehicle_id').append(`<option value="${rowData.vehicle_number}">${rowData.Vehicle_Num}</option>`);
}
$j('#vehicle_id').val(rowData.vehicle_number);

// For Renter
if ($j(`#render_name option[value="${rowData.renter_name}"]`).length === 0) {
    $j('#render_name').append(`<option value="${rowData.renter_name}">${rowData.customer_name}</option>`);
}
$j('#render_name').val(rowData.renter_name);
    ...
    $j('#oillevel').append(`<option value="${rowData.oillevel}">${rowData.oillevel}</option>`);
}
    ...
    $j.ajax({
        ...
        success: function(response) {
            ...
            if (response.status) { // ✅ This allows both true or 'success'
                ...
                select.find('option:not(:first)').remove(); // Keep the first option "-- Select Expense --"
